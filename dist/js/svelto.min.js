...
!function(t,e,r,o,n){"use strict";t.Pointer={pressDuration:300,doubleTapInterval:300,flickDuration:150,motionThreshold:5};var i=["tap","dbltap","press","dragstart","dragmove","dragend","flick"],a="pointer";e.each(i,function(e){var r=a+e;t.Pointer[e]=r,t.fn[e]=function(t){return t?this.on(r,t):this.trigger(r)}});var s,u,c,d,g,l,h,m,v,X,Y,p,f=t.browser.is.touchDevice?"touchstart":"mousedown",P=t.browser.is.touchDevice?"touchmove":"mousemove",b=t.browser.is.touchDevice?"touchend touchcancel":"mouseup mouseleave",w=t(o.documentElement),D=0,T=function(e,r){var o=t.Event(e);return o.originalEvent=r,o.isPointerEvent=!0,o},k=function(r){s=t.eventXY(r),g=r.target,l=t(g),h=r.timeStamp||e.now(),v=!1,p=setTimeout(e.wrap(r,E),t.Pointer.pressDuration),l.trigger(T(t.Pointer.dragstart,r),{startXY:s}),w.on(P,M),w.on(b,y)},E=function(e){l.trigger(T(t.Pointer.press,e))},M=function(e){clearTimeout(p),u=t.eventXY(e),c={X:u.X-s.X,Y:u.Y-s.Y},(Math.abs(c.X)>t.Pointer.motionThreshold||Math.abs(c.Y)>t.Pointer.motionThreshold)&&(v=!0,l.trigger(T(t.Pointer.dragmove,e),{startXY:s,moveXY:u,deltaXY:c}))},y=function(r){clearTimeout(p),d=t.eventXY(r),c={X:d.X-s.X,Y:d.Y-s.Y},g===r.target&&("touchend"===r.type||"mouseup"===r.type&&0===r.button)&&(m=r.timeStamp||e.now(),t.browser.is.touchDevice&&v||(l.trigger(T(t.Pointer.tap,r)),m-D<=t.Pointer.doubleTapInterval&&l.trigger(T(t.Pointer.dbltap,r)),D=m),v&&m-h<=t.Pointer.flickDuration&&(Math.abs(c.X)>Math.abs(c.Y)?(X="horizontal",Y=c.X>0?1:-1):(X="vertical",Y=c.Y>0?1:-1),l.trigger(T(t.Pointer.flick,r),{startXY:s,endXY:d,deltaXY:c,orientation:X,direction:Y}))),w.off(P,M),w.off(b,y),l.trigger(T(t.Pointer.dragend,r),{startXY:s,endXY:d,deltaXY:c})};w.on(f,k)}(jQuery,_,window,document);
...
!function(e,s,t,n,o){"use strict";e.factory("svelto.expander",{options:{classes:{open:"open"},selectors:{expander:".expander",toggler:".expander-toggler"},callbacks:{open:s.noop,close:s.noop}},_variables:function(){this.$expander=this.$element,this.$togglers=this.$expander.find(this.options.selectors.toggler),this._isOpen=this.$expander.hasClass(this.options.classes.open)},_events:function(){this._on(this.$togglers,e.Pointer.tap,this.toggle)},isOpen:function(){return this._isOpen},toggle:function(e){s.isBoolean(e)||(e=!this._isOpen),e!==this._isOpen&&(this._isOpen=e,this.$expander.toggleClass(this.options.classes.open,this._isOpen),this._trigger(this._isOpen?"open":"close"))},open:function(){this.toggle(!0)},close:function(){this.toggle(!1)}}),e(function(){e(".expander").expander()})}(jQuery,_,window,document);
...
!function(c,e,t,h,n){"use strict";c.factory("svelto.checkbox",{options:{callbacks:{checked:e.noop,unchecked:e.noop}},_variables:function(){this.$checkbox=this.$element,this.$input=this.$checkbox.find("input")},_init:function(){var c=this.$checkbox.hasClass("checked");this.get()?c||this.$checkbox.addClass("checked"):c&&this.$checkbox.removeClass("checked")},_events:function(){this._on("click",function(){this.toggle()}),this._on(!0,"change",this._handler_change)},_handler_change:function(){var c=this.get();this.$checkbox.toggleClass("checked",c),this._trigger(c?"checked":"unchecked")},get:function(){return this.$input.prop("checked")},toggle:function(c){var t=this.get();e.isUndefined(c)&&(c=!t),c!==t&&(this.$input.prop("checked",c).trigger("change"),this._trigger(c?"checked":"unchecked"))},check:function(){this.toggle(!0)},uncheck:function(){this.toggle(!1)}}),c(function(){c(".checkbox").checkbox()})}(jQuery,_,window,document);
...
!function(i,s,t,p,l){"use strict";i.factory("svelto.flippable",{options:{classes:{flip:"flipped"},selectors:{front:".flippable-front",back:".flippable-back",flipper:".flippable-trigger"},callbacks:{font:s.noop,back:s.noop}},_variables:function(){this.$flippable=this.$element,this.$front=this.$flippable.find(this.options.selectors.front),this.$back=this.$flippable.find(this.options.selectors.back),this.$flippers=this.$flippable.find(this.options.selectors.flipper),this.isFlipped=this.$flippable.hasClass(this.options.classes.flip)},_events:function(){this._on(this.$flippers,i.Pointer.tap,this.flip)},flip:function(i){s.isBoolean(i)||(i=!this.isFlipped),i!==this.isFlipped&&(this.isFlipped=i,this.$flippable.toggleClass(this.options.classes.flip,this.isFlipped),this._trigger(this.isFlipped?"back":"front"))},front:function(){this.flip(!1)},back:function(){this.flip(!0)}}),i(function(){i(".flippable").flippable()})}(jQuery,_,window,document);
...
!function(t,i,o,n,s){"use strict";var e=[];t.noty=function(o){return o=i.isString(o)?{body:o}:o||{},o.buttons&&(o.type="action"),new t.svelto.noty(o)},t.factory("svelto.noty",{templates:{base:'<div class="noty {%=(o.transparentize ? "transparentize" : "")%} {%=o.type%} {%=(o.type !== "action" ? "actionable" : "")%} {%=o.color%} {%=o.css%}"><div class="infobar">{% if ( o.img ) { %}<img src="{%=o.img%}" class="noty-img infobar-left" />{% } %}{% if ( o.title || o.body ) { %}<div class="infobar-center">{% if ( o.title ) { %}<p class="infobar-title">{%#o.title%}</p>{% } %}{% if ( o.body ) { %}{%#o.body%}{% } %}</div>{% } %}{% if ( o.buttons.length === 1 ) { %}<div class="infobar-right">{% include ( "svelto.noty.button", o.buttons[0] ); %}</div>{% } %}</div>{% if ( o.buttons.length > 1 ) { %}<div class="noty-buttons multiple centered">{% for ( var i = 0; i < o.buttons.length; i++ ) { %}{% include ( "svelto.noty.button", o.buttons[i] ); %}{% } %}</div>{% } %}</div>',button:'<div class="button {%=(o.color || "white")%} {%=(o.size || "small")%} {%=(o.css || "")%}">{%#(o.text || "")%}</div>'},options:{anchor:{y:"bottom",x:"left"},title:!1,body:!1,img:!1,buttons:[],type:"alert",color:"black",transparentize:!0,css:"",ttl:3500,autoplay:!0,timerMinimumRemaining:1e3,classes:{open:"open"},selectors:{button:".noty-buttons .button, .infobar-right .button"},animations:{remove:t.ui.animation.normal},callbacks:{open:i.noop,close:i.noop}},_variables:function(){this.$noty=this.$element,this.$buttons=this.$noty.find(this.options.selectors.button),this.timer=!1,this._isOpen=!1,this.neverOpened=!0},_init:function(){this.options.autoplay&&this.open()},___click:function(){"action"!==this.options.type&&this._on(t.Pointer.tap,this.close)},___buttonClick:function(){i.each(this.options.buttons,function(i,o){this._on(this.$buttons.eq(o),t.Pointer.tap,function(t,n){i.onClick&&i.onClick.apply(this.$buttons.get(o),[t,n])===!1||this.close()})},this)},_init_timer:function(){"action"!==this.options.type&&i.isNumber(this.options.ttl)&&this.options.ttl!==1/0&&(this.timer=t.timer(this.close.bind(this),this.options.ttl,!0),e.push(this.timer))},___hover:function(){var t=this;this.$noty.hover(function(){i.each(e,function(t){t.pause()})},function(){i.each(e,function(i){i.remaining(Math.max(t.options.timerMinimumRemaining,i.remaining()||0)),i.play()})})},___flick:function(){"action"!==this.options.type&&this._on(t.Pointer.flick,function(t,i){"horizontal"===i.orientation&&this.close()})},__keydown:function(i){i.keyCode===t.ui.keyCode.ESCAPE&&(i.stopImmediatePropagation(),this.close())},isOpen:function(){return this._isOpen},open:function(){this._isOpen||(this._frame(function(){t(".noty-queues."+this.options.anchor.y+" .noty-queue."+this.options.anchor.x).append(this.$noty),this._frame(function(){this.$noty.addClass(this.options.classes.open)})}),this.neverOpened&&(this.___click(),this.___flick(),this.___buttonClick(),this.___hover(),this.neverOpened=!1),this._init_timer(),this._on($document,"keydown",this.__keydown),this._isOpen=!0,this._trigger("open"))},close:function(){this._isOpen&&(this.$noty.removeClass(this.options.classes.open),this._delay(function(){this.$noty.detach()},this.options.animations.remove),this.timer&&(i.pull(e,this.timer),this.timer.stop()),this._off($document,"keydown",this.__keydown),this._isOpen=!1,this._trigger("close"))}}),t(function(){$body.append('<div class="noty-queues top"><div class="noty-queue expanded"></div><div class="noty-queues-row"><div class="noty-queue left"></div><div class="noty-queue center"></div><div class="noty-queue right"></div></div></div><div class="noty-queues bottom"><div class="noty-queues-row"><div class="noty-queue left"></div><div class="noty-queue center"></div><div class="noty-queue right"></div></div><div class="noty-queue expanded"></div></div>')})}(jQuery,_,window,document);
...
!function(o,s,i,t,e){"use strict";o.factory("svelto.infobar",{options:{selectors:{closer:".infobar-closer"},callbacks:{close:s.noop}},_variables:function(){this.$infobar=this.$element,this.$closers=this.$infobar.find(this.options.selectors.closer)},_events:function(){this._on(this.$closers,o.Pointer.tap,this.close)},close:function(){this.$infobar.remove(),this._trigger("close")}}),o(function(){o(".infobar").infobar()})}(jQuery,_,window,document);
!function(s,o,t,i,e){"use strict";s.factory("svelto.modal",{options:{classes:{open:"open"},selectors:{trigger:".modal-trigger",closer:".modal-closer",background:".modal-background"},callbacks:{open:o.noop,close:o.noop}},_variables:function(){this.$modal=this.$element,this.id=this.$modal.attr("id"),this.$triggers=s(this.options.selectors.trigger+'[data-modal="'+this.id+'"]'),this.$closers=this.$modal.find(this.options.selectors.closer),this.$background=this.$modal.next(this.options.selectors.background),this._isOpen=this.$modal.hasClass(this.options.classes.open)},_events:function(){this._on(this.$triggers,s.Pointer.tap,this.open),this._on(this.$closers.add(this.$background),s.Pointer.tap,this.close)},__keydown:function(o){o.keyCode===s.ui.keyCode.ESCAPE&&(o.stopImmediatePropagation(),this.close())},isOpen:function(){return this._isOpen},toggle:function(s){o.isBoolean(s)||(s=!this._isOpen),s!==this._isOpen&&(this._isOpen=s,this.$modal.toggleClass(this.options.classes.open,this._isOpen),this[this._isOpen?"_on":"_off"]($document,"keydown",this.__keydown),this._trigger(this._isOpen?"open":"close"))},open:function(){this.toggle(!0)},close:function(){this.toggle(!1)}}),s(function(){s(".modal").modal()})}(jQuery,_,window,document);
...
!function(i,t,e,n,s){"use strict";i.factory("svelto.navbar",{options:{flickableArea:20,callbacks:{open:t.noop,close:t.noop}},_variables:function(){this.$navbar=this.$element,this.id=this.$navbar.attr("id"),this.$closers=this.$navbar.find(".navbar-closer, + .navbar-background"),this.$triggers=i('.navbar-trigger[data-navbar="'+this.id+'"]'),this.direction=this.$navbar.data("direction"),this.isFlickable=this.$navbar.hasClass("flickable"),this.opened=this.$navbar.hasClass("opened")},_events:function(){this._on(this.$closers,i.Pointer.tap,this.close),this._on(this.$triggers,i.Pointer.tap,this.open),this.isFlickable&&this._on($document,i.Pointer.flick,this._handler_flick)},_handler_flick:function(i,t){if(!this.opened)switch(this.direction){case"left":case"right":"horizontal"===t.orientation&&("left"===this.direction?1===t.direction&&t.startXY.X<=this.options.flickableArea&&this.open():"right"===this.direction&&-1===t.direction&&$window.width()-t.startXY.X<=this.options.flickableArea&&this.open());break;case"top":case"bottom":"vertical"===t.orientation&&("top"===this.direction?-1===t.direction&&t.startXY.Y<=this.options.flickableArea&&this.open():"bottom"===this.direction&&1===t.direction&&$window.height()-t.startXY.Y<=this.options.flickableArea&&this.open())}},toggle:function(i){t.isUndefined(i)&&(i=!this.opened),i!==this.opened&&(this.opened=i,this.$navbar.toggleClass("opened",this.opened),this._trigger(this.opened?"open":"close"))},open:function(){this.toggle(!0)},close:function(){this.toggle(!1)}}),i(function(){i(".navbar").navbar()})}(jQuery,_,window,document);
...
!function(t,e,s,i,n){"use strict";t.factory("svelto.stepper",{options:{min:0,max:100,value:0,step:1,selectors:{decreaser:".stepper-decreaser",input:"input",increaser:".stepper-increaser"},callbacks:{change:e.noop,increase:e.noop,decrease:e.noop}},_variables:function(){this.$stepper=this.$element,this.$decreaser=this.$stepper.find(this.options.selectors.decreaser),this.$input=this.$stepper.find(this.options.selectors.input),this.$increaser=this.$stepper.find(this.options.selectors.increaser),this.options.value=this._sanitizeValue(this.options.value)},_init:function(){this._updateButtons()},_events:function(){this._on(!0,this.$input,"input change",this.__inputChange),this._on("mouseenter",this.__arrowsIn),this._on("mouseleave",this.__arrowsOut),this._on(this.$decreaser,t.Pointer.tap,this.decrease),this._on(this.$increaser,t.Pointer.tap,this.increase)},_sanitizeValue:function(t){var s=Number(t);t=e.isNaN(s)?0:s;var i=t%this.options.step;return t=t-i+(i>=this.options.step/2?this.options.step:0),e.clamp(this.options.min,t,this.options.max)},_updateInput:function(){this.$input.val(this.options.value).trigger("change")},_updateButtons:function(t){var e=this.options.value===this.options.min,s=this.options.value===this.options.max;t===this.options.min||e?this.$decreaser.toggleClass("disabled",e):(t===this.options.max||s)&&this.$increaser.toggleClass("disabled",s)},_update:function(t){this._updateInput(),this._updateButtons(t)},__inputChange:function(){this.set(this.$input.val())},__arrowsIn:function(t){this._on($document,"keydown",this.__keydown)},__arrowsOut:function(t){this._off($document,"keydown",this.__keydown)},__keydown:function(e){switch(e.keyCode){case t.ui.keyCode.UP:this.increase();break;case t.ui.keyCode.DOWN:this.decrease()}},get:function(){return this.options.value},set:function(t){if(t=Number(t),!e.isNaN(t)&&(t=this._sanitizeValue(t),t!==this.options.value)){var s={previous:this.options.value,value:t};return this.options.value=t,this._update(s.previous),this._trigger("change",s),void this._trigger(s.previous<s.value?"increase":"decrease",s)}this.$input.val()!==String(this.options.value)&&this._updateInput()},increase:function(){this.set(this.options.value+this.options.step)},decrease:function(){this.set(this.options.value-this.options.step)}}),t(function(){t(".stepper").each(function(){var e=t(this);e.stepper({min:Number(e.data("min")||0),max:Number(e.data("max")||100),value:Number(e.find(".stepper-input").val()||0),step:Number(e.data("step")||1)})})})}(jQuery,_,window,document);
...
!function(t,s,i,n,e){"use strict";t.factory("svelto.tabs",{options:{selectors:{buttons_wrp:".tabs-buttons",buttons:".button-wrp > .label",button_active_class:"active",indicator:".tabs-buttons-indicator",containers_wrp:".tabs-containers",containers:"> .container",container_active_class:"active"},indicator_delay:40,callbacks:{select:s.noop}},_variables:function(){this.$tabs=this.$element,this.isVertical=this.$tabs.hasClass("vertical"),this.$tabs_buttons=this.$tabs.find(this.options.selectors.buttons_wrp),this.$buttons=this.$tabs_buttons.find(this.options.selectors.buttons),this.$indicator=this.$tabs.find(this.options.selectors.indicator),this.$tabs_containers=this.$tabs.find(this.options.selectors.containers_wrp),this.$containers=this.$tabs_containers.find(this.options.selectors.containers);var t=this.$buttons.filter("."+this.options.selectors.button_active_class).first();t=t.length>0?t:this.$buttons.first(),this.prev_index=0,this.current_index=this.$buttons.index(t)},_init:function(){this.select(this.current_index,!0)},_events:function(){this._on(this.$tabs_buttons,"click",this.options.selectors.buttons,this._hander_button_click),this._on($window,"resize",this._positionate_indicator)},_hander_button_click:function(s,i){var n=this.$buttons.index(t(i));this.select(n)},_positionate_indicator:function(){var t=this.$buttons.filter("."+this.options.selectors.button_active_class),s=t.position();if(this.isVertical){var i=this.$tabs_buttons.height();this._delay(function(){var n=s.top+(0===this.$buttons.index(t)?1:0);this.$indicator.css("top",100*n/i+"%")},this.current_index>this.prev_index?this.options.indicator_delay:0),this._delay(function(){var n=i-s.top-t.height()+(this.$buttons.index(t)===this.$buttons.length-1?1:0);this.$indicator.css("bottom",100*n/i+"%")},this.current_index>this.prev_index?0:this.options.indicator_delay)}else{var n=this.$tabs_buttons.width();this._delay(function(){var i=s.left+(0===this.$buttons.index(t)?1:0);this.$indicator.css("left",100*i/n+"%")},this.current_index>this.prev_index?this.options.indicator_delay:0),this._delay(function(){var i=n-s.left-t.width()+(this.$buttons.index(t)===this.$buttons.length-1?1:0);this.$indicator.css("right",100*i/n+"%")},this.current_index>this.prev_index?0:this.options.indicator_delay)}},select:function(t,s){(this.current_index!==t||s)&&(this.$buttons.eq(this.current_index).removeClass(this.options.selectors.button_active_class),this.$buttons.eq(t).addClass(this.options.selectors.button_active_class),this.$containers.eq(this.current_index).removeClass(this.options.selectors.container_active_class),this.$containers.eq(t).addClass(this.options.selectors.container_active_class),this.current_index!==t&&(this.prev_index=this.current_index,this.current_index=t),this._positionate_indicator())}}),t(function(){t(".tabs").tabs()})}(jQuery,_,window,document);
!function(t,i,s,e,a){"use strict";t.factory("svelto.tagbox",{templates:{tag:'<div class="label-tag tagbox-tag" data-tag-value="{%=o.value%}"><div class="label {%=o.color%} {%=o.size%} {%=o.css%}"><span>{%=o.value%}</span><div class="sub right gray actionable tagbox-tag-remover"><i class="icon">close</i></div></div></div>'},options:{init:"",tags:[],tag:{minLength:3,color:"",size:"",css:"outlined"},characters:{forbidden:["<",">",";","`"],separator:",",inserters:[t.ui.keyCode.ENTER,t.ui.keyCode.TAB]},sort:!1,escape:!0,deburr:!1,selectors:{input:"input.hidden",partial:"input.tagbox-partial, .tagbox-partial input",tags:".tagbox-tags",tag:".tagbox-tag",tagLabel:"span",tagRemover:".tagbox-tag-remover"},callbacks:{change:i.noop,add:i.noop,remove:i.noop,empty:i.noop}},_variables:function(){this.$tagbox=this.$element,this.$tags=this.$tagbox.find(this.options.selectors.tags),this.$input=this.$tagbox.find(this.options.selectors.input),this.$partial=this.$tagbox.find(this.options.selectors.partial)},_init:function(t){this.add(this.options.init,t)},_events:function(){this._on(this.$partial,"keypress keydown",this.__keypressKeydown),this._on(this.$partial,"paste",this.__paste),this._on(t.Pointer.tap,this.__clickOnEmpty),this._on(t.Pointer.tap,this.options.selectors.tagRemover,this.__clickOnTagRemover)},_sanitizeTag:function(t){return t=i.trim(t),this.options.escape&&(t=i.escape(t)),this.options.deburr&&(t=i.deburr(t)),t},_getTagHtml:function(t){return this._tmpl("tag",i.merge({value:t},this.options.tag))},_clearPartial:function(){this.$partial.val("").trigger("change")},_updateInput:function(){this.$input.val(this.options.tags.join(this.options.characters.separator)).trigger("change")},_add:function(s){var e=i.trim(s),s=this._sanitizeTag(s);if(e.length<this.options.tag.minLength)e.length>0&&t.noty("`"+s+"` is shorter than "+this.options.tag.minLength+" characters");else{if(!i.contains(this.options.tags,s)){this.options.tags.push(s),this.options.sort&&this.options.tags.sort();var a=this._getTagHtml(s);if(1===this.options.tags.length)this.$tags.prepend(a);else if(this.options.sort){var o=this.options.tags.indexOf(s);0===o?this.$tagbox.find(this.options.selectors.tag).first().before(a):this.$tagbox.find(this.options.selectors.tag).eq(o-1).after(a)}else this.$tagbox.find(this.options.selectors.tag).last().after(a);return!0}t.noty("`"+s+"` is a duplicate")}return!1},_remove:function(t,s){t.remove(),i.pull(this.options.tags,s)},__keypressKeydown:function(s){var e=this.$partial.val();if(i.contains(this.options.characters.inserters,s.keyCode)||s.keyCode===this.options.characters.separator.charCodeAt(0)){var a=this.add(e);a&&this._clearPartial(),s.preventDefault()}else if(s.keyCode===t.ui.keyCode.BACKSPACE){if(0===e.length&&this.options.tags.length>0){var o=this.$tagbox.find(this.options.selectors.tag).last(),n=!t.hasCtrlOrCmd(s);this.remove(o,n),s.preventDefault()}}else i.contains(this.options.characters.forbidden,String.fromCharCode(s.keyCode))&&(t.noty("The character you entered is forbidden"),s.preventDefault())},__paste:function(t){this.add(t.originalEvent.clipboardData.getData("text")),t.preventDefault()},__clickOnTagRemover:function(i,s){var e=t(s).parents(this.options.selectors.tag);this.remove(e)},__clickOnEmpty:function(i){e.activeElement===this.$partial[0]||t(i.target).is("input, "+this.options.selectors.tagLabel)||this.$partial.focus()},get:function(){return i.clone(this.options.tags)},add:function(t,s){i.isArray(t)&&(t=i.flatten(t).join(this.options.characters.separator));var e=i.clone(this.options.tag),a=t.split(this.options.characters.separator),o=i.map(a,this._add,this),n=i.compact(o).length>0;if(n&&(this._updateInput(),!s)){this._trigger("change",{previous:e,tags:i.clone(this.options.tags)});var r=i.filter(a,function(t,i){return o[i]});this._trigger("add",r)}return n},remove:function(s,e,a){if(s instanceof t)var o=[s],n=[s.data("tag-value")];else{var o=[],n=[];i.isArray(s)&&(s=i.flatten(s).join(this.options.characters.separator)),s=s.split(this.options.characters.separator);for(var r=0,h=s.length;h>r;r++){var g=this._sanitizeTag(s[r]),p=this.$tagbox.find(this.options.selectors.tag+'[data-tag-value="'+g+'"]');1===p.length&&(o.push(p),n.push(g))}}if(n.length>0){for(var l=i.clone(this.options.tags),r=0,h=n.length;h>r;r++)this._remove(o[r],n[r]);this._updateInput(),1===n.length&&e===!0&&this.$partial.val(n[0]).trigger("change"),a||(this._trigger("change",{previous:l,tags:i.clone(this.options.tags)}),this._trigger("remove",n),0===this.options.tags.length&&this._trigger("empty"))}},clear:function(t){if(this.options.tags.length>0){var s={previous:i.clone(this.options.tags),tags:[]};this.options.tags=[],this.$tagbox.find(this.options.selectors.tag).remove(),this._clearPartial(),this._updateInput(),t||(this._trigger("change",s),s.previous.length>0&&this._trigger("remove",s.previous),this._trigger("empty"))}},reset:function(){var t=i.clone(this.options.tags);if(this.clear(!0),this._init(!0),!i.isEqual(t,this.options.tags)){this._trigger("change",{previous:t,tags:i.clone(this.options.tags)});var s=i.difference(this.options.tags,t);s.length>0&&this._trigger("add",s);var e=i.difference(t,this.options.tags);e.length>0&&this._trigger("remove",e),0===this.options.tags.length&&this._trigger("empty")}}}),t(function(){t(".tagbox").each(function(){var i=t(this);i.tagbox({init:i.find("input").val()})})})}(jQuery,_,window,document);
